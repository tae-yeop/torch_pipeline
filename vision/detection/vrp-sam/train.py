...
def train(args, epoch, model, sam_model, dataloader, optimizer, scheduler, training):
    ...
    for idx, batch in enumerate(dataloader):
        # dict_keys(['query_img', 'query_mask', 'query_name', 'org_query_imsize', 'support_imgs', 'support_masks', 'support_names', 'class_id'])
        batch = utils.to_cuda(batch) # args.condition : 'scribble'
        protos, _ = model(args.condition, batch['query_img'], batch['support_imgs'].squeeze(1), batch['support_masks'].squeeze(1), training)
        # protos torch.Size([2, 50, 256])
        low_masks, pred_mask = sam_model(batch['query_img'], batch['query_name'], protos) # <-여기 쿼리 이름이 들어가는 이유는?
        logit_mask = low_masks # torch.Size([2, 1, 512, 512])
        
        pred_mask = torch.sigmoid(logit_mask) > 0.5 # torch.bool
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', type=str, default='/purestorage/project/tyk/9_Animation/Segmentation/Datasets_HSN')
